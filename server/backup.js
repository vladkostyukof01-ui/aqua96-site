...
// 3) скопировать нужный файл из data/backups/ поверх data/aqua96.db,
...
const dbPath = path.join(__dirname, '..', 'data', 'aqua96.db');
...
const backupPath = path.join(backupDir, `aqua96-${timestamp}.db`);
...
const files = fs.readdirSync(backupDir)
  .filter(f => f.startsWith('aqua96-') && f.endsWith('.db'))
  .sort();
